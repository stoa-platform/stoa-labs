...
|---|---|---|
| les deux `assert` (`TEAM_UNDEFINED`, `TEAM_CONFIRMED`) | `apim_ss_require_team` | `team-name.yml:37`, `verify.yml:75` |
| le travail réel (`GET /accessProfiles`, `POST /assets/team`) | `ss_team_name \| length > 0` | `team.yml:45` |
| la garde de visibilité | `ss_team_name \| length > 0` | `main.yml:54` |
...
`-e apim_ss_team="$TEAM"` (`Jenkinsfile.selfservice:315,332`,
...
**Re-verifie le 2026-08-04 : rien n'a bouge.** Le travail sur la DCR a ajoute des
fichiers au role sans deplacer ces points ; les ancres ci-dessus sont a jour a
cette date, mais elles derivent — les retrouver par `grep -n`, pas par numero.
...
**La policy existe déjà — la reprendre, pas la réinventer.** Revue du
2026-08-04 : `scripts/setup-vault-userpass.sh:75-97` crée `deploy-<tenant>` avec
lecture sur tout le sous-arbre du tenant **et écriture sur `apps/*`**. Cette
écriture n'est pas décorative : c'est le mode OAuth2 `internal`, où celui qui
déploie une application y stocke le client généré par la gateway. Une policy en
lecture seule casserait ces déploiements en silence. Le rôle reprend donc la
forme verbatim, et en devient le **propriétaire unique** — le script d'amorçage
cesse de l'écrire, sans quoi deux sources divergeront tôt ou tard sans que rien
ne le signale.
...
**Révision du 2026-08-04 — ADR-081 pèse contre la piste Jenkins.** Une première
version de cette section privilégiait **Jenkins comme surface**, au motif que le
mécanisme de demande en attente est déjà prouvé (`PAUSED_PENDING_INPUT`, build
#22, identité alice) et qu'il **enregistre qui a répondu**, ce que le mail ne
fait pas.

ADR-081 (2026-08-03) décide que la décision humaine vit dans le **merge**, pas
dans une étape du CI, et son deuxième argument vise directement ce motif : *« la
décision vit dans un log de build — soumis à la rotation et à la purge des
anciens builds. Pour une banque, la PR est la pièce d'audit. Un log Jenkins n'en
est pas une : il est volatil par conception. »* L'enregistrement dont on
créditait Jenkins n'a donc pas la durabilité qu'on lui prêtait.

La distinction ci-dessus tient toujours, et ADR-081 **confirme** la première
moitié : pour l'approbation d'un changement, la surface est Git, et la PR est
l'acte d'autorisation. La seconde moitié n'est pas tranchée par ADR-081 — une
souscription runtime n'est pas un changement suivi dans Git, donc elle sort de
son périmètre — mais l'argument d'auditabilité, lui, s'y transporte sans effort.

**Conséquence pour le spec dédié à venir :** la piste Jenkins n'est plus
« privilégiée ». Elle doit être ré-argumentée contre ADR-081 — en montrant où la
pièce d'audit durable serait produite — ou abandonnée au profit d'une surface qui
laisse une trace non volatile. Ce qui reste acquis : les approbateurs n'atteignent
pas la gateway, seul Jenkins l'atteint, et la surface doit donc être quelque chose
qu'ils atteignent **et** qui atteint la gateway.
...
| Policy `deploy-<team>` : forme existante reprise, propriété au rôle | L'écriture sur `apps/*` porte le mode OAuth2 internal ; deux écrivains = divergence garantie (revue 2026-08-04) |
| Piste Jenkins pour l'approbation runtime : déclassée | ADR-081 — un log de build est volatil par conception, la PR est la pièce d'audit |
