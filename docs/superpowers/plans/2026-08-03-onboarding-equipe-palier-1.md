...
**Files:**
- Modify: `ansible/roles/apim_selfservice_app/tasks/main.yml:23-25` et `:53-55` et `:106-107`
- Modify: `ansible/roles/apim_selfservice_app/tasks/team.yml:45`
- Modify: `ansible/roles/apim_selfservice_app/tasks/verify.yml:74`
...
`apim_selfservice_app/tasks/verify.yml` — l'assert (ligne 74) :
...
# La policy borne la lecture au SEUL chemin de l'equipe. C'est la douve : se
# reclamer de l'equipe X exige de savoir lire deploy/X/wm-admin (ADR-077).
- name: "Vault : policy {{ onb.policy }} (lecture bornee au chemin de l'equipe)"
  ansible.builtin.uri:
    ...
    body:
      policy: |
        path "{{ apim_ss_vault_kv_mount }}/data/{{ [apim_ss_vault_prefix, 'deploy', onb.team] | select | join('/') }}/*" {
          capabilities = ["read"]
        }
        path "{{ apim_ss_vault_kv_mount }}/metadata/{{ [apim_ss_vault_prefix, 'deploy', onb.team] | select | join('/') }}/*" {
...
- [ ] **Step 4: Prouver contre un Vault de lab**
