...
1. **Fusion & push — FAIT le 2026-08-06.** Branche `feat/onboarding-equipe-palier-1` poussée sur **origin** (`129dcd7`) et **gitea** (branche), et **gitea `main` avancé en fast-forward** vers `d41b816` (réconciliation palier 3 + travail gitea existant). Fusion sur `origin/main` : à la discrétion de l'exploitant.

2. **Gate Gitea — LEVÉ le 2026-08-06.** `ci/stoa-labs@main` porte désormais le palier 3 (`d41b816` = branche rebasée sur le travail gitea existant, fast-forward, 0 fichier perdu). Le contrôle qui restait « strictement non prouvé en conditions réelles » — le job `team-publish` comme **vrai pipeline Jenkins** — est **PROUVÉ** : une vraie PR d'API (`banking-demo/accounts-api#4`, `api-test` v1.0.0) mergée par oscar a déclenché le vrai webhook → job `team-publish` clonant le **vrai `ci/stoa-labs@main`** → `MERGE_IDENTITY_OK` (oscar merge ET répond à la pause) → réconciliation Gitea OK → apply → `api-test` v1.0.0 **publiée, active, cloisonnée sur `banking-demo`** (build #2 SUCCESS). Le commentaire de statut est passé de `⚠` (1er essai) à `✅` sous le même marqueur `team-publish-build` — le comportement 2-marqueurs validé en réel. **PRÉREQUIS GATEWAY découvert par ce passage réel** : publier une API producteur exige `enableTeamWork=true` sur la gateway cible ; sinon le rôle refuse **fail-closed** (`TEAMS_DISABLED`, rien écrit, jamais silencieux) — c'est un **geste d'ADMIN** (`PUT /rest/apigateway/configurations/extended {"enableTeamWork":"true"}`), hors pipeline.

3. **Migration : assigner les APIs `Default` à une équipe.** Conséquence directe de la fermeture de la capture : sur la gateway, **toute API en `Default`** (les 12 APIs du lab, `provisioning` comprise) devient **non ré-appliquable par une équipe** tant qu'un admin ne lui a pas assigné une équipe réelle (`POST /assets/team`). `Default` ne vaut pas appartenance — c'est la fermeture même. À prévoir avant que les équipes ne publient sur une gateway existante. Outil fourni : **`scripts/assign-api-team.sh`** (idempotent, `--list` inventorie les APIs `Default`-only, `--team <équipe> <api…>` assigne et **vérifie par relecture** ; les deux pièges 10.15 gérés — `assetType` obligatoire, POST 200-menteur ; refuse d'assigner à un profil système). N'assigner QUE les APIs qu'une équipe possède légitimement — les APIs d'infra plateforme (`provisioning`) se laissent en `Default`.
