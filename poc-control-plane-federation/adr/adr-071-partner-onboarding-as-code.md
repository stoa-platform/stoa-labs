...
**Date :** 2026-06-11.
**Contexte technique :** gateway webMethods API Gateway 10.15.
**Lié à :**.
...
> **Frontière nette** : l'API **valide et commit**, point. **STOA ne configure pas le flux**, ne touche pas le data-plane ; **Git est la source de vérité** ; ce qui est en Git est **non-secret et auditable par PR** (cert public + IP + tokens de test) — **les secrets (client secret, clés privées) vivent dans Vault/PAM**, jamais en Git.
...
- **reuse-first** : si l'onboarding est manuel et propre à wM, il n'est ni portable ni neutre vendor.
- **hors data-plane** : un onboarding « API → écrit direct dans wM » remettrait un composant STOA sur le chemin de contrôle runtime du partenaire.
- **Git source de vérité** : sans manifeste en Git, il n'y a **pas** de source de vérité auditable de **qui peut appeler quoi** — donc pas de douve sur l'onboarding.
...
- **Hors data-plane** : l'API d'onboarding **n'appelle jamais** wM. Elle écrit un commit ; la **CI / `labctl`** convergent la gateway. Aucun composant STOA n'est sur le chemin du partenaire.
- **Reuse-first** : réutiliser `labctl subscribe`, l'adapter wM, le moteur write-through-Git de `internal/governance` (commit signé, re-lecture depuis Git, idempotence). Ne **rien** réécrire.
...
2. **API qui écrit directement dans wM (REST admin).** *Rejeté.* Remet un composant STOA sur le chemin de contrôle runtime (rupture le principe hors-data-plane) ; pas de diff revu (rupture du modèle 4 yeux / le principe Git-source-de-vérité) ; couple l'API à wM (rupture neutralité le principe reuse-first) ; pas de réversibilité « `git revert` ».
...
**L'API ne touche JAMAIS wM** (`Service` n'a **aucun** client gateway — vérifié par construction). Ensuite, la **CI** (ou un `labctl subscribe` sur le manifeste) converge la gateway. C'est exactement la séparation le principe hors-data-plane : l'API **gouverne** (commit), la CI **converge** (projection), STOA **n'est pas** sur le chemin de la transaction.
...
- **Hors data-plane strict** : l'API ne fait qu'un commit ; aucun composant STOA sur le chemin du partenaire. La projection est confiée à la CI/labctl.
- **Source de vérité unique en Git** : la douve s'étend à **qui peut appeler quoi** ; coût de sortie élevé (toute la posture d'accès partenaire est en Git, neutre vendor).
- **Reuse-first** : réutilise `labctl subscribe`, l'adapter wM, le moteur write-through-Git de `internal/governance` — **zéro** réécriture du chemin de commit.
...
| 5 | Cible de l'API | **write-through-Git uniquement** (commit) | **jamais** la gateway |
