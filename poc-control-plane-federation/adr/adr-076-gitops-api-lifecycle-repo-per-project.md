...
3. **La sécurité pilotée par l'intégrité (état initial → corrigé → ENFORCÉ).** `classification` *était* un enum validé mais non dérivé (`labctl/internal/uac/uac.go` exclut `classification`/`required_policies` de la projection ; `validate.go` ne dérivait rien). **Comblé** : `internal/render` intégré à `governance.ValidateUAC`, **fail-closed** (`INTEGRITY_INCONSISTENT`). **Écart résiduel fermé (2026-07-03, goal A1)** : `labctl apply` porte désormais le gate d'enforcement — quand un contrat UAC (`api.yaml`) est colocalisé au manifeste (ou nommé via `--uac`), (a) **pré-check statique** : chaque target doit déclarer les knobs du bundle dérivé, sinon `[INTEGRITY_UNFULFILLED]` et **rien n'est écrit** ; (b) **read-back gateway post-publish** : l'état RELU (strategy + scope mapping + action IAM toutes-règles + throttle LMT + logInvocation global + transport) doit confirmer le bundle, sinon `[ENFORCEMENT_UNCONFIRMED]`. Preuve : `scripts/test-integrity-enforce.sh` **31/31 live**, dont la contre-épreuve : un sabotage hors-bande de l'action IAM AND (que le projecteur ne converge jamais) est attrapé par le read-back. Sémantique assumée : un verdict UNCONFIRMED **ne désactive pas l'API** (gate niveau pipeline, cohérent rollback-sans-DELETE) — remédiation = pipeline rouge + rapport de réconciliation (défense en profondeur, TOCTOU couvert au prochain apply/reconcile) ; le gate garantit bundle(classification **déclarée**) ⊆ enforced, l'anti-downgrade de la classification reste l'écart #1 (gouvernance centrale).
...
**Impact enum (décision #1) — localisé, pas de cross-repo produit :** `H/VH/VVH` → `VH/H/M` touche **4 sites** dans ce repo — `labctl/cmd/governance-api/uac_contract_v1_schema.json:109` (enum + description l.110), `labctl/internal/governance/validate.go:34` (`classificationEnum`) + `:79` (message d'erreur), et 1-2 fixtures de test. `VH` reste valide (tests inchangés), `VVH` devient invalide, `M` est ajouté. **Ce n'est PAS** le changement Rust+Python cross-repo du produit — celui-là n'intervient qu'en Phase 8 (réconciliation STOA).

> **Correction (2026-07-04).** L'hypothèse « aucune donnée on-disk n'utilise VVH » était **fausse** : la migration n'avait touché que labctl/BFF, laissant **console-light** entièrement sur l'ancien enum (schéma ajv, `UacClassification`, 3 pages UI, seed) **et** un contrat vivant `banking-demo/payments-initiation` en `VVH` → validation BFF `422` à chaque édition/publish. Corrigé : console-light migré vers `VH/H/M` (mapping par criticité `H→M`, `VH→H`, `VVH→VH`) et le seed vivant recomposé en `VH` (commité sur `main` du repo governance). Suite Playwright 18/18. Les branches de promotion en vol restent sûres (le merge conserve le `VH` de `main`).
...
## Modèle de sécurité par couches — PROUVÉ vs ASPIRATIONNEL (revue adversariale banque centrale)
...
| **Sécurité = f(intégrité)** | ✅ **DÉRIVÉE + VALIDÉE + ENFORCÉE à l'apply (gate read-back fail-closed, A1 2026-07-03) + RÉCONCILIÉE ; limites trial annotées, jamais surclamées** | Le moteur `internal/render` dérive `required_policies` **fail-closed** (intégré `governance.ValidateUAC` → `INTEGRITY_INCONSISTENT`). Un **rapport de réconciliation** (Ansible, read-backs LIVE) classe chaque policy `enforced/degraded/blocked` par API — pour **ne jamais surclamer**. État réel accounts-read VH/external : oauth2=ENFORCED\* (3/4 ; audience fail-open trial), audit-log=ENFORCED (global logInvocation, métadonnées seules), ip-allowlist=DÉGRADÉ (consommateur), **rate-limit=ENFORCED** (throttle "Traffic Optimization" sur le stage LMT, rejet 429, in-gateway non fail-open — projeté par `labctl apply` depuis le bloc `rateLimit` du manifeste), **mtls=ENFORCED par-API à l'IAM** (modèle *port request + barrière par-API*, choix client) : le listener :5543 en `clientAuth=request` laisse passer le no-cert au transport ; le mTLS est enforcé au stage **IAM** par une action = **`oAuth2Token` AND `httpsCertificate`** (`allowAnonymous=false`), projetée par `labctl apply` depuis `inboundAuth.mtls: true`. No-cert → **401 à l'IAM** (par-API, pas par-port) ; cert sans token → 401. Vérifié live. Cert mappé au consumer (`identifiers.go` httpsCertificate). Le mode `request` du listener + l'import CA dans `DEFAULT_IS_TRUSTSTORE` restent la **couche IS-admin/Ansible** (hors REST ; keystore hors allowlist ADR-075). |
| **mTLS (VH)** | ✅ **ENFORCED par-API à l'IAM (accounts-read)** — modèle port `request` | Listener :5543 en `clientAuth=request` (cert optionnel au transport, toutes les APIs joignables) ; le mTLS d'une API VH est enforcé au stage **IAM** : action Identify & Authorize = `oAuth2Token` **AND** `httpsCertificate` (`allowAnonymous=false`), projetée par `labctl apply` (`inboundAuth.mtls: true`). No-cert → 401 à l'IAM (par-API, pas par-port) ; cert sans token → 401. Cert client mappé au consumer (`labctl subscribe` publicCertRef → identifier `httpsCertificate`). Deux actions partagées coexistent : OAuth2-seule (H/M) et OAuth2+cert (VH), distinguées par la règle cert. **Couche IS-admin livrée en Ansible** (`ansible/is-mtls-setup.yml`, pur `ansible.builtin`) : (1) listener HTTPS `clientAuth=request` via `PUT /ports` idempotent + read-back fail-closed ; (2) CA cliente importée dans `DEFAULT_IS_TRUSTSTORE` via `keytool` (idempotent par check d'alias) + handler restart IS (le JKS n'est relu qu'au boot). Logique REST validée live (read-only) ; keytool+restart prouvés manuellement. Secrets en Vault (ADR-074) ; en prod → stoa-infra/ansible. |
...
1. **Tag-spoof / classification auto-déclarée — ✅ FERMÉ (goal A5, 2026-07-04).** Le niveau était posé par l'équipe **dans son propre repo** (spoofable). **Correctif livré** : registre de classification **central** (`stoa-platform-ci/governance/classifications.yaml`, repo plateforme non éditable ; en prod = repo `data-governance`). `labctl apply` reçoit la source (`LABCTL_CLASSIFICATION_SOURCE`) + l'identité PROJET **non-éditable** (`LABCTL_PROJECT` = `PROJECT_NAME` du pipeline). Le bundle est **dérivé du central** (autoritaire) ; l'`api.yaml` projet n'est qu'une **référence** : plus faible que le central → `[CLASSIFICATION_SPOOFED]`, mauvais tenant → `[CLASSIFICATION_SPOOFED]`, API absente/empruntée à un autre projet → `[CLASSIFICATION_UNGOVERNED]`. **Ancre anti-spoof** : la clé de lookup est **(owner, api)** où `owner` vient du pipeline, pas de l'`api.yaml` — un projet ne voit que ses propres entrées (ferme le trou « pointer une ligne plus faible » de la review). Prouvé `scripts/test-classification-central.sh` **11/11** (2 repos → bundles VH vs H différents ; downgrade/emprunt/tenant/ungoverned refusés ; un faux `governance/` planté dans le repo projet est **ignoré** — seul le chemin plateforme fait foi). Le PR-gate projet reste aveugle au central (feedback ; l'autorité est le gate plateforme = séparation des devoirs). Onboarding d'une API = 1 PR gouvernance (sérialisé, pas self-service).
