...
**Maturité technique :** ✅ Livré & prouvé (2026-07-03) — moteur `render` + gate `INTEGRITY_INCONSISTENT` (validate) **+ gate d'enforcement à l'apply** : pré-check statique `INTEGRITY_UNFULFILLED` (le manifeste implémente le bundle dérivé, sinon rien n'est écrit) et read-back gateway `ENFORCEMENT_UNCONFIRMED` (l'état RELU confirme le bundle, sinon refus) — `scripts/test-integrity-enforce.sh` **31/31 live** dont contre-épreuve sabotage. Restent documentés : audience fail-open trial (3/4), ip-allowlist dégradé (consommateur), anti-spoof de la classification (écart #1, goal A5).
**Date :** 2026-07-01.
