...
## Extension 2026-09-02 (A3, GOAL cd-applications) — le second objet

Le ticket d'entrée de G4 (« la lecture d'`envs/<env>/wm-admin` EST l'autorisation ») ne servait que la promotion d'API (`team-promote.sh` §7.b). L'apply d'**application** (`selfservice-app-deploy`) lisait, lui, un credential de **tenant** (`deploy/<tenant>/wm-admin`) et sélectionnait le palier par un en-tête `X-Environment` : le palier était un en-tête, pas un credential. A3 porte le mécanisme de cet ADR au second objet, sans en créer aucun autre :

- **Le ticket, avant tout contact gateway.** `scripts/selfservice-palier-gate.sh` (chargé par le Jenkinsfile depuis `origin/main`, jamais depuis l'arbre pinné au `MERGE_SHA` — sinon le levier de repli A6 pinnerait la garde avec) lit `envs/<env>/wm-admin` avec le token nominatif de la pause ⇒ `PALIER_FERME` sinon ; le préflight de joignabilité et les deux plays viennent **après**. Le F4-canari est rejoué sur cette voie (policy `apply-rec` révoquée ⇒ garde fermée, zéro connexion ; restaurée ⇒ un hit).
- **Le ticket doit être non inscriptible** (`sys/capabilities-self` : `create/update/delete/patch` absents ⇒ sinon `TICKET_INSCRIPTIBLE`) : un gabarit de chemin pointé sur le sous-arbre inscriptible du tenant ne fabrique pas un ticket. Et les deux gabarits de sous-chemin doivent porter `__ENV__` (`CREDS_SUB_SANS_PALIER`) — un credential qui ne varie pas par palier ne décide d'aucun palier.
- **L'équipe est décidée par le token** (policies `deploy-<tenant>`), le manifeste et le knob ne peuvent que concorder (`TEAM_NON_PORTEE`) : l'invariant de `team-name.yml` (« l'appelant ne choisit pas sous quelle équipe son application est cloisonnée »), que le chemin du credential du tenant portait jusqu'ici, est repris sur le token.
- **Le terminus, par position** : `TERMINUS_SANS_VOIE` tant qu'aucune voie directe n'est déclarée (`APIM_TERMINUS_BASE`, sans défaut — « dire sa cible est volontaire », G7), puis le credential. Aucun refus par nom de palier : l'ouvrir (A7) est une déclaration + un credential + une porte (A4), jamais un edit.
- **Le grant humain** est le geste de cet ADR (§2, « ajout de l'humain au groupe annuaire `apim-apply-rec` ») : sur le lab, `DEPLOYERS_DEV=alice DEPLOYERS_REC=alice scripts/setup-deployer-groups.sh` — explicite, jamais un défaut ; le read-back « la demandeuse n'est pas déployeuse » du poseur ne s'applique qu'aux paliers dont la porte déclare un `deployerGroup` (l'axe qu'il protège), jamais à `fourEyes`.

**Limites mesurées, à lire avant l'audit** : le ticket est **par palier, pas par objet** (`apply-rec` ouvre `rec` aux applications ET aux APIs) ; sur le lab mono-gateway, les comptes `wm-<env>-admin` sont tous administrateurs de la même 10.15 — la rétention est prouvée côté Vault, la ségrégation au plan de données est topologique ; `capabilities-self` dépend de la policy `default` (`CAPACITES_INVERIFIABLES` sans elle, fail-closed).

Preuves : hors ligne `scripts/test-selfservice-palier-a3.sh` (stub Vault + canari + câblage + mutations, `make lint-ci` [12/12]) ; live `scripts/test-palier-retention-live.sh` ⑦ (la porte du GOAL : l'identité de l'apply de `rec` ne lit aucun `envs/int/*`) et `scripts/test-a3-live.sh` (builds réels). Spec : `docs/superpowers/specs/2026-09-02-a3-credential-du-seul-palier-design.md`.
