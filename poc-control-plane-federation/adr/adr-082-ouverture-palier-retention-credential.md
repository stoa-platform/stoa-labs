...
Conséquence directe : **« ouvrir rec à un humain » = un geste Vault de l'exploitant** (`--mint apply-rec` puis grant nominatif `vault write auth/userpass/users/<u> token_policies=+apply-rec`, ou mapping de groupe LDAP `apim-apply-rec` → policy `apply-rec`). L'état sorti du script est « tout fermé ». **Un pipeline compromis ne peut pas se l'accorder** — il n'a pas la main sur Vault. Les AppRoles sont posés sans consommateur immédiat (le consommateur est l'apply de promotion, G5) ; un AppRole non minté n'élargit aucune surface, aucun secret-id ne circule.
