...
|---|---|---|
| `scripts/test-archive-store.sh` (branchée `make lint-ci`) | hors-ligne : push idempotent, fetch re-haché, digest faux, token jamais en argv/URL | incluse dans `make lint-ci [6/7]` |
...
| `make lint-ci` | Jenkinsfiles compilés + shellcheck + épreuves de tout G3/G4/G5 | `[1/7]`→`[7/7]`, rc=0 |
