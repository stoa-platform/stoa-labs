...
status: "Acté pour le MÉCANISME, prouvé par les portes. Porte du GOAL VERTE : 54/54 par chacun des deux moteurs contre le wM réel (scripts/test-promote-verb-live.sh — GUID iso, 0-coupure 3550/3550 et 4074/4074 requêtes 200 pendant l'import, UPDATE_FORBIDDEN rejouée live ET unitaire). Fidélité du mock 22/22 contre le mock ET contre le wM réel teamWork actif (scripts/test-archive-promotion.sh). Câblage hors-ligne 90/0 (scripts/test-team-promote-wiring.sh), branché make lint-ci [7/7]. Reste des DÉCISIONS CLIENT et des limites nommées : la politique d'attribution des grants par palier (ADR-082, décision n°2), la parité des deux moteurs (G8), le client OAuth partagé hors-prod, la conversion du pipeline governance."
...
- **La chaîne E2E complète n'a pas tourné en un seul geste Jenkins sur ce lab.** La couche Jenkins (webhook → build → pause d'approbation) n'a jamais été verte ici : le gitea du lab date d'avant G3 et le push exploitant qui referait pointer le webhook dessus reste à faire. Ce qui EST prouvé, c'est chaque script du chemin rejoué directement contre le lab vivant (T10) — pour le moteur labctl, en chaîne script-par-script ; le moteur Ansible vers un palier mocké n'est rejouable que depuis le correctif de fidélité base64 (`0a1ac86`), postérieur à cet E2E. Le rejeu du geste Jenkins lui-même reste à la charge de l'exploitant, suivant la checklist du rapport T10.
