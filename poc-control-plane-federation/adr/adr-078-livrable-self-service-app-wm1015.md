...
Chez le client, le **port data-plane est en Deny-by-Default** : chaque API publiée doit être **ajoutée à l'allow-list du port**. C'est l'**Access Mode du listener Integration Server** — surface **différente** de l'API d'admin apigateway (~~le REST `/ports` ne l'expose pas~~ — **FAUX, corrigé le 2026-09-04**, voir l'encadré ci-dessous), pilotée par le form WmRoot `security-ports-editaccess.dsp`.

> **⚠ CORRECTION DU 2026-09-04 — deux affirmations de ce paragraphe sont fausses** (spike P0 du GOAL « posture par exposition », `scripts/spike-p0-posture.py`, 45/0 sur la 10.15 réelle) :
>
> 1. **`/rest/apigateway/ports/{listenerKey}/accessMode` EXISTE.** `POST` porte le mode (`{"accessMode":"deny"}`), `PUT` porte la liste (`{"services":[…]}`). Les **deux surfaces pilotent la MÊME liste**, vérifié dans les deux sens : une entrée écrite en REST apparaît dans le formulaire WmRoot, et un `addNode` du formulaire est relu par le REST. Différence de sémantique, elle bien réelle : le **REST réécrit la liste EN BLOC** (aucune opération additive — 13 entrées → 1 après un `PUT` d'une seule), là où le **formulaire est additif**. Pour un pipeline concurrent, le formulaire reste donc le chemin sûr — mais pour la raison de la concurrence, pas parce que le REST manquerait.
> 2. **Cette allow-list ne garde PAS les APIs.** Mesuré par mutation sur un listener jetable : port en `deny`, liste ne contenant que des services IS, `/invoke/<service non listé>` → **403 `PortAccessException [ISS.0084.9101]`**, `/invoke/<service listé>` → **200**… et `/gateway/<api>/<version>/<ressource>` → **200, réponse du backend incluse**. Le mode d'accès d'un port de l'Integration Server garde `/invoke`, **pas le dispatch de l'API Gateway**. Inscrire une API publiée dans cette liste ne la protège donc de rien, et l'en retirer ne la ferme pas.
>
> Conséquence : le mécanisme de `tasks/port-access.yml` est **correct pour ce qu'il fait** (il inscrit bien une entrée, il la relit, il est fail-closed) mais il est **braqué sur la mauvaise surface** pour l'objectif annoncé. Le levier réellement par-API est `entryProtocolPolicy` sur l'étage `transport` — prouvé au même spike : une API servie sur un listener HTTPS est refusée avec « Transport protocol not supported » tant que sa propre policy n'accepte que `http`. Ne pas activer `apim_ss_port_manage` en croyant fermer une exposition. Décision : `tasks/port-access.yml` (opt-in `apim_ss_port_manage`, après l'activate) fait **read → add idempotent → read-back fail-closed** (`PORT_ALLOWLIST_CONFIRMED`) ; play standalone `is-port-access.yml` pour la création d'env. **Le rôle ne FLIPPE JAMAIS le mode** (risque de lockout du data-plane) : le passage en Deny est fait à la création de l'env côté client. *Prouvé live 2026-07-15 : POST addNode/deleteNode + read-back basic-auth sans CSRF.* **Caveats** (§ Écarts) : allow-list par **service IS `folder:service`** (le port rejette les URLs data-plane — testé), mapping API→service = config gateway du client ; **CSRF guard** OFF sur le trial (à gérer si activé côté client).
...
4. **🟠 `/applications` non cloisonné par team — REQUALIFIÉ le 2026-07-31 (mesure sur la gateway du cluster).** Le constat du spike #1 (delete cross-team → 204, register d'une API invisible → 201) portait sur des applications restées en team `Default`. Mesuré depuis : une app **assignée** à une team via `POST /assets/team {assetType:"Application"}` est **cloisonnée nativement** — cross-team `GET`/`DELETE` → **401 ×5**, absente de `GET /applications`, la suppression **n'a pas lieu**, le propriétaire conservant son accès (témoins verts au même instant). Ce qui reste vrai, et ce qui reste bloquant :
