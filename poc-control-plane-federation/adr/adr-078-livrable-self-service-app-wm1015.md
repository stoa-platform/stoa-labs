...
1. **🔴 `ipAllowlist` décoratif (fail-open silencieux).** `labctl` écrit l'identifier `ipAddressRange` sur l'application mais ne pose **jamais** de règle IAM par IP (`wantMode()` ne produit que `oAuth2Token`/`jwtClaims`, + `AND httpsCertificate` en mTLS). Conséquence : l'`ipAllowlist` du manifeste d'onboarding est **stocké mais inopposé**. **C'est un fail-open** — le type de défaut qu'un audit bancaire trouve. **Chemin critique de la V1.** Correctif = généraliser l'action IAM `AND` (§ décision 5) + preuve X/X reproduisant la matrice A-D.
