...
| `ci/Jenkinsfile.provision-plan` | **Plan lecture seule d'une demande** (A0) : PR `provision/*` ouverte/resynchronisée (webhook, token `stoa-provision-plan`) → `scripts/provision-plan.sh` → verdict commenté sur la PR. Garde `provision/*` avant tout agent. Coquille : `ci/jenkins/provision-plan.job.xml`. |
| `ci/Jenkinsfile.provisioning-request` | **Voie machine OIG/CLI2** (A0) : webhook `stoa-provision-request` (7 clés `REQ_*`) → `scripts/provision-request.sh`, **sans lister un seul champ** (hérite de tout enrichissement du script). Coquille : `ci/jenkins/provisioning-request.job.xml`. |
