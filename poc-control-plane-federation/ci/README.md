...
| **Rollback** | `ci/Jenkinsfile.rollback` | **AUCUN trigger** — paramètres `PROMOTION_ID`+`REASON`+`CHANGE_REF` (requis seulement si le gate du palier l'exige) | `POST .../promotions/{id}/rollback` sur governance-api (revert **Git** audité, token compte de service) — palier RELU dans la réponse, jamais saisi — puis **re-apply idempotent du palier de la promotion** (terminus en direct, paliers intermédiaires via `wm-admin-<env>`) **+ smoke d'état N-1**. Jamais de DELETE gateway (ADR-085). |
| **Repli d'application** | `ci/Jenkinsfile.app-rollback` | **AUCUN trigger** — formulaire `APP`+`ENV` (chaîne entière)+`REASON`+`CHANGE_REF` (requis si la porte du palier l'exige) | ouvre une **PR** `provision/<app>-<env>` dont `per_env.<env>` et le cert du palier redeviennent ceux du merge PRÉCÉDENT (N-1) ; la chaîne de tous les jours l'applique (ADR-089 : même GUID, même clé ; `REPLI_EN_COURS`/`REPLI_PERIME` gardent la fenêtre). Coquille `ci/jenkins/app-rollback.job.xml`, pose `JOBS=app-rollback BOOTSTRAP_JOBS=app-rollback scripts/setup-provision-jobs.sh`. |
