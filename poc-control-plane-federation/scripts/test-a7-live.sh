...
console_order "$TMP/ss.$S_NUM.console" "équipe : '$TEAM' — celle du manifeste mergé (la porte vers prod nomme le déployeur 'apim-operator-prod') ; tenants du porteur : aucun" "déclaration déployeur : 'oscar' porte 'operator-deploy'" "PALIER_VIA=direct" "PALIER_BASE=$TERMINUS_ADMIN" "palier ouvert : envs/prod/wm-admin" "préflight de joignabilité : $TERMINUS_ADMIN/health" \
  && ok "5.5 garde du terminus : équipe de Git (tenants : aucun) < oscar porte operator-deploy < voie directe sur le terminus < ticket envs/prod/wm-admin < préflight" || ko "5.5 garde : $(grep -E 'équipe|déclaration|PALIER_VIA|PALIER_BASE|palier ouvert|préflight' "$TMP/ss.$S_NUM.console" | head -6 | tr '\n' ' ')"
...
gw -H 'Accept: application/zip' -o "$TMP/export.zip" -w '%{http_code}' "$GW_ADMIN/archive?apis=$G_API" > "$TMP/export.hc"; [ "$(cat "$TMP/export.hc")" = 200 ] && [ -s "$TMP/export.zip" ] || die "PREREQUIS : export de $G_API depuis la 10.15 (HTTP $(cat "$TMP/export.hc"))"
...
TA=$(t_api "$REQ_API" "$REQ_API_VER")
[ "$(tcode)" = 200 ] && [ "${TA%% *}" = "$G_API" ] && [ "${TA#* }" = True ] && ok "5.8 geste producteur : archive 10.15 → terminus (HTTP 200) ⇒ l'API porte le MÊME GUID $G_API sur le terminus, isActive True (le verbe ADR-079, sens réel→mock)" || die "PREREQUIS : import au terminus (HTTP $(tcode)) — terminus : '$TA' (attendu $G_API True ; jamais un activate silencieux)"
