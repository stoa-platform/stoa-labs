...
# delete any existing 'Keycloak' KM so a re-run is clean
...
[[ -n "$existing" ]] && curl -sk -X DELETE "$WSO2/api/am/admin/v4/key-managers/$existing" -H "Authorization: Bearer $ATOK" -o /dev/null
...
for attempt in 1 2 3 4 5; do
  ...
  [[ "$code" == "200" ]] && break
  sleep 5   # the KM needs a moment to load into the gateway runtime after restart
done
