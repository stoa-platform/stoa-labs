...
grep 'selfservice-palier-gate.sh' "$TMP/jfs.code" | pipe_q 'REFUS_DETAIL_OUT="\$WORKSPACE/.a3-refus-detail"' && ok "C.2 la garde A3 reçoit REFUS_DETAIL_OUT sur sa ligne d'appel" || bad "C.2 REFUS_DETAIL_OUT absent de la ligne de la garde"
...
L_G=$(grep -n 'selfservice-palier-gate.sh" || exit 1' "$TMP/jfs.code" | head -1 | cut -d: -f1); L_P=$(grep -n 'apim_preflight "\$APIM_API_BASE"' "$JFS" | head -1 | cut -d: -f1); L_C=$(grep -n 'ansible/selfservice-app.yml \\' "$TMP/jfs.code" | head -1 | cut -d: -f1); L_V=$(grep -n 'ansible/selfservice-app-verify.yml \\' "$TMP/jfs.code" | head -1 | cut -d: -f1)
