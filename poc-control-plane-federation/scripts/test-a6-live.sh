...
[ "$PARAMS" = "APP ENV REASON CHANGE_REF DEBUG FORGE_TOKEN" ] || die "PREREQUIS : formulaire app-rollback non posé ou antérieur à L4 (paramètres : '$PARAMS', attendu APP ENV REASON CHANGE_REF DEBUG FORGE_TOKEN) — amorcer le job"
...
[ "$ENVS_FORM" = "$CHAIN_ALL" ] && ok "0.4 job app-rollback posé, formulaire APP ENV REASON CHANGE_REF DEBUG FORGE_TOKEN (A7 + L4), ENV == chaîne entière ($CHAIN_ALL)" || ko "0.4 formulaire ENV='$ENVS_FORM' ≠ chaîne '$CHAIN_ALL'"
