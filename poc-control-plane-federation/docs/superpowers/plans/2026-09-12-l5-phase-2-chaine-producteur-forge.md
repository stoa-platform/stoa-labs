...
**Interfaces:**
- Produces: `setup-team-repos.sh <owner/repo> [--print] [--no-hook] [--no-protect]` — knobs `FORGE_KIND`, `GIT_HOST`, `FORGE_SECRET` (Gitea : `write:organization,write:repository` ; GitLab : PAT `api` Owner du groupe), `TEAM_PUBLISH_WEBHOOK_URL` (défaut gwt `http://jenkins:8080/generic-webhook-trigger/invoke?token=stoa-team-publish`, défaut gitlab `http://jenkins:8080/project/team-publish`), `TEAM_PROMOTE_WEBHOOK_URL` (**gitlab seulement**, défaut `http://jenkins:8080/project/team-promote` : le GitLab Plugin a une URL par job, il faut un hook PAR récepteur ; sous gwt le hook unique au token partagé sert les deux jobs — Ruling 6), `WEBHOOK_KIND` (gwt|gitlab : forme du hook), `GIT_BASE` (branche à protéger, défaut `main`). Le `--print` liste chaque hook sur sa ligne `HOOK=` (deux lignes sous gitlab). Sorties `--print` : `MODE=print HOST= KIND= REPO= HOOK= PROTECT=` ; refus `REPO_REQUIS`, `SECRET_FORGE_REQUIS`, `FORGE_KIND_INCONNU`, `CREATION_ECHEC : … (HTTP n)`.
...
- Rayon mesuré (session L6) : 18 appelants de `forge_api_init` ; les suites nomment `FORGE_KIND` ; `provision-plan.sh` et `team-apply-identity.sh` le reçoivent de leur Jenkinsfile ; les outils EXEMPTS (`setup-team-repos.sh`, `setup-gitlab-lab.sh`, `repo-protection.sh`) n'appellent pas l'init — rien à faire pour eux ; **`seed-governance-chain.sh` ENTRE dans le rayon depuis Task 12** (read-back par `forge raw`) : son en-tête documente `FORGE_KIND` requis, sans défaut réintroduit ; `scripts/lib/archive-store.sh` (`_as_url`, Task 13) n'appelle pas l'init mais garde `${FORGE_KIND:-gitea}` : l'aligner dans cette tâche (`FORGE_KIND` absent ⇒ `_as_fail "FORGE_KIND_REQUIS …"`) pour qu'aucun chemin de la lib ne défaute en silence. La porte à portée dérivée de la session L6 (`ci/lint-forge-knobs.sh`, lit `ROUTES`) a trouvé un **douzième** Jenkinsfile sans knobs (`ci/Jenkinsfile.provisioning-request`, voie machine) : son commit pose les knobs dans 7 Jenkinsfile et retire le repli `?: 'gitea'` des 12 porteurs — sans quoi ce refus serait inatteignable. `test-team-apply-wiring.sh` §3quinquies désoude volontairement `FORGE_KIND` (`env -u`) et reste VERTE (son assertion porte sur `FORGE_IDENTITES_ILLISIBLES`) : ne pas la lire comme une régression.
...
- [ ] **Step 0 : la globale du lab AVANT le refus (critique)** — entre le commit L6 (repli `?: 'gitea'` retiré des Jenkinsfile) et ce refus, `FORGE_KIND` arrive VIDE aux scripts et la lib défaute encore : tout marche. Dès que le refus est là, un lab SANS la globale voit TOUS les jobs refuser. Donc, d'abord : `FORGE_KIND=gitea … scripts/setup-jenkins-globals.sh` sur le Jenkins du lab, puis relire la globale (le script sait la relire ; sinon `curl -u … /scriptText` avec `Jenkins.instance.globalNodeProperties`) et ne passer au Step 1 qu'avec `FORGE_KIND=gitea` LU. Vérifier aussi que le commit L6 est bien dans `main` : `grep -c FORGE_KIND ci/Jenkinsfile.{team-request,api-request,api-promote-request,api-promote-export,team-publish,team-promote,provisioning-request}` = 7 × ≥ 1.
