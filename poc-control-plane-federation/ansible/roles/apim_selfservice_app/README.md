...
| `per_env.<env>.backend_key_ref` | **clé backend stockée sur l'application** (variante client) : sous-chemin **KV v2** de la clé — Git ne porte **jamais** la valeur. Lue à l'apply, posée en identifier **`token`** (singulier — l'énumération 10.15 rejette `tokens` en 400). Le **nom** de l'entrée n'est pas opposable — la policy du client sélectionne **par type**, d'où **une seule** entrée `token` — mais c'est **ce que l'UI affiche** : le défaut est donc le **libellé du type**, `"Token"` (convention du swagger officiel : `{"name":"Token","key":"token"}`, `{"name":"Username","key":"httpBasicAuth"}`). Surchargeable par **`backend_key_identifier_name`** ; le merge restant par **dimension**, renommer **remplace** l'entrée au lieu d'en ajouter une. Absente/vide dans Vault = échec (`BACKEND_KEY_MISSING`, rien n'est posé) ; `token` listé dans `enforce` = échec (`BACKEND_KEY_ENFORCED` — la clé backend n'est pas une identité entrante). `verify` compare la valeur live à celle de Vault (`BACKEND_KEY_UNCONFIRMED`). Champ lu dans l'entrée : `backend_key_field`, défaut `api_key` — **le nom du champ dans Vault doit correspondre exactement** (une entrée dont la clé s'appelle `api-key` avec un tiret est lue vide → `BACKEND_KEY_MISSING` alors que l'entrée existe : le HTTP 200 du message d'erreur signe justement une entrée trouvée mais un champ absent). Preuve hors ligne : `scripts/test-backend-key.sh` (27/27) — **PAR ENV** |
...
| `auth` | bloc OAuth2 **opt-in** (absent ⇒ aucune stratégie posée). `mode: idp` = le client vit sur l'IdP, `claim {name,value}` l'identifie (**forme multi-palier A1, 2026-09-02** : la racine ne porte que `claim: { name }`, la **valeur** — le `client_id` du palier — vit sous `per_env.<env>.auth.claim.value` ; la fusion racine ⊕ per_env la recompose, cf. `consumer-auth.yml`), **`audience` obligatoire** (= celle de l'API). `mode: internal` = la gateway est l'AS (`local`) et **`audience` est OPTIONNELLE** : l'AS local n'en exige pas, et ce runtime n'oppose de toute façon pas `aud` (`EVIDENCE.md` §Preuve 5 bis). Vide ⇒ la clé est **omise** du corps et de l'entrée Vault, jamais envoyée à `""`. Preuve hors ligne : `scripts/test-auth-audience.sh` (13/13) |
| `auth` (mode `internal`) — **le client est émis par la GATEWAY** | La stratégie porte un **`dcrConfig`** et **aucun `clientId`** (« Generate credentials » de l'UI = dynamic client registration) : la gateway crée le client et le rôle le range dans Vault. **Où lire le secret — mesuré LIVE sur la 10.15 (2026-08-03), et la doc 11.0 est trompeuse ici** : `POST /strategies` **et** `GET /strategies/{id}` rendent le `clientSecret` **masqué** (32 astérisques) ; **seul `GET /strategies` (la LISTE) le rend en clair**. Le rôle capture donc par la liste — s'y tromper écrit des astérisques dans Vault sous un apply vert. Le `clientId`, lui, est en clair partout (`vault_sub`, tenant+env-scopé) avec `grant_types`/`client_type`/`scopes` pour que le consommateur rejoue un `client_credentials` sans redécouvrir la config. Poser un `clientId` bascule la stratégie sur « ce client existe déjà » — la gateway n'émet **rien** (le bug corrigé le 2026-08-03). Knobs : **`client_type`** (défaut `CONFIDENTIAL` ; `PUBLIC` + `client_credentials` = refus `CLIENT_PUBLIC_AVEC_CLIENT_CREDENTIALS`), **`grant_types`** (défaut `["client_credentials"]`, énumération du produit sinon `GRANT_TYPES_INVALIDES`), **`scopes`** — **à laisser vide dans le cas nominal** : le rôle lit les scopes que l'AS **local** publie (`alias.scopes`, schéma `AuthServerAlias`) et retient **celui par défaut** (`SCOPE_PAR_DEFAUT`). Un scope déclaré qui n'existe pas sur l'AS = refus `SCOPE_INCONNU` (sinon le client est lié à rien et ne se voit refuser qu'au **runtime**, chez le consommateur) ; plusieurs scopes sans défaut évident = refus `SCOPE_AMBIGU` avec la liste (on ne choisit pas les droits du client en silence) ; AS sans aucun scope = `SCOPES_ABSENTS` annoncé, non bloquant, `token_lifetime`, `refresh_count`, `redirect_uris`, `application_type`. **Idempotent** : re-apply compare *désiré ⊆ live* et n'écrit rien s'il n'y a pas de dérive (`DCR_CONVERGED`) — le credential en vol n'est jamais roté par accident ; la convergence est un **read-modify-write** (un PUT nu effacerait `clientRegistration`). Si le build **masque** le secret en relecture, le rôle **ne réécrit pas** Vault et vérifie que la valeur y est (`INTERNAL_CLIENT_KEPT`) ; absente des deux côtés = `INTERNAL_CLIENT_SECRET_MANQUANT`, rien n'est écrit. Rotation explicite : `PUT /strategies/{id}/refreshCredentials` — attention, **mesuré live : elle régénère le `clientId` AUSSI**, ce n'est pas un simple renouvellement de secret (tout ce qui détient l'ancien clientId casse). Preuves : `scripts/test-internal-dcr.sh` **38/38** hors ligne + **run LIVE sur `apigateway-trial:10.15`** (`failed=0`, secret réel écrit en Vault, 2e apply `DCR_CONVERGED` avec couple inchangé, objets jetables supprimés) |
