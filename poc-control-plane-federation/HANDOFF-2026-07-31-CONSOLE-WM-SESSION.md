...
Le dépôt disait « trial expirée, ~25-30 min ». Le détail exact, lu sur le pod :
...
Et à chaque démarrage de l'Integration Server :

```
[ISS.0001.0002C] Integration Server License key expired or invalid.
[ISS.0001.0004C] Integration Server will shutdown in 30 minutes, unless a valid
                 license key is provided.
```

**Les CronJobs ne renouvellent rien : ils devancent une auto-extinction à 30
min.** Le clustering est dans les droits de la licence — mais la clé est morte
...
gateways, `licenseKey.xml` du conteneur, admin REST
`configurations/dataspace`, et l'état ArgoCD après synchronisation. Commits
`5155730` (correctif) et `94d8c52` (réserves au spike)._
