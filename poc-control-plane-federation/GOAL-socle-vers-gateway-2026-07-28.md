...
**État au 2026-07-30 : cadré, spécifié, planifié et outillé — PAS FERMÉ.** Les trois portes retenues (P-a invocation data-plane réelle par le nom public, P-b fermeture de la surface d'admin, P-c worker-3 réduit à Caddy) sont **toutes en aval de deux gestes exploitant** refusés à l'agent, et tentés dans la session : le **merge de la PR stoa #2825** (qui pose le backend réel `backend-dev` et épingle la ClusterIP) et **toute écriture dans `/etc/caddy/Caddyfile`** (terminaison TLS de la flotte entière). Détail et commandes : `HANDOFF-2026-07-30-F5-BASCULE.md`.

**Le cadrage a requalifié trois entrées du jalon, par la mesure :**
- **Il n'y a aucun trafic public d'API à préserver** : `transactionalevents` de worker-3 = **0 document**, aucune API jamais invoquée en data-plane ; les seuls accès externes tracés sont 29 événements d'admin depuis 3 IP de l'exploitant. C'est pourquoi la porte retenue est une **invocation réelle** (sur un backend posé en cluster), et non « le trafic est servi » — qu'une gateway ne servant rien tiendrait.
- **La « migration des ~109 Mo d'ES » n'est pas un sujet** : 92 % sont des logs d'audit produits par le cycle de redémarrage, et les 7 APIs (61 ko) sont des artefacts de spike sans consommateur. **Décision (spéc § D6) : on archive à froid, on ne restaure rien** — restaurer un `tar` contournerait la chaîne GitOps qu'on vient de prouver (ADR-076). Ce n'est pas un oubli.
- **Le cycle trial coupe 150 s par cycle de 20 min** (mesuré : 07:20:23 → 07:22:54, sonde à 5 s), soit **87,5 % de disponibilité** — et non « ~15 min de service par cycle », qui **surestimait le trou d'un facteur 2**. Corrigé dans la PR #2825. Un seul cycle observé : à rejouer sur deux cycles avant de graver le chiffre.

**Un durcissement est apparu** : `dev-wm.gostoa.dev/rest/apigateway/apis` rend **401 publiquement** aujourd'hui — l'admin REST wM est sur l'Internet ouvert derrière un basic auth, alors que la contre-épreuve F3 avait mesuré 6/6 refus pour la gateway du cluster. Basculer Caddy à l'identique l'aurait **re-publié en silence**. F5 ne publie que `/gateway/*` (spéc § D3).
