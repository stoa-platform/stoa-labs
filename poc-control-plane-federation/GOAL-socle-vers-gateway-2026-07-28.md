...
| ~~Rotation du mot de passe bootstrap `ci`~~ | rapports lot 1 | **soldée — F4, 2026-07-29** (rayon d'action mesuré d'abord : tirages d'images anonymes, aucun `imagePullSecrets` ; ancien mdp → 401, nouveau → 200, nouveau mdp root-only sur worker-1 ; chaîne re-prouvée verte après rotation) |
| ~~Valeur du mot de passe bootstrap encore en clair dans les docs~~ | handoff F1 | **soldée — 2026-07-31** (33 occurrences retirées de 15 fichiers ; substituant `<MDP_CI>` dans les plans, `$(cat /root/gitea-ci-pass)` dans les scripts rejouables). Reste : l'historique Git du dépôt public porte encore la valeur — identifiant **révoqué**, dépôt de lab, non repris |
...
| ~~`team:` natif dans labctl (moteur unique ADR-076)~~ | spéc F4 § D1 | **SANS OBJET depuis le 2026-08-02** — E1 (D1) fait passer la publication au rôle Ansible `apim_publish_api`, qui porte désormais tout le durcissement (`assetType`, relecture fail-closed, équipe venue de la chaîne, liste blanche du manifeste), prouvé par cinq portes. Cela **renverse la décision F4 §D1**, et il faut le dire ainsi : maintenir deux moteurs, c'était corriger deux fois les mêmes quirks wM et n'en prouver qu'un. `labctl` reste la spec vérifiée parquée. |
...
| ~~Spike : deux répliques wM 10.15 à redémarrages décalés sur un ES partagé~~ | spéc F5 § D9 | **EXÉCUTÉ ET CONCLUANT le 2026-07-30.** Mesuré côté public sur 63 min : **99,1 % de disponibilité contre ~85 %** en mono-réplique — la coupure passe de 120–235 s par cycle à **moins de 5 s** (sous la résolution de la sonde). Ignite était bien le mécanisme (`pg-cluster.xml`), et le rééquilibrage redouté ne se manifeste pas à cette échelle. **L'hypothèse à réfuter ne l'a pas été.** Deux Deployments d'une réplique, labels `rotation: a|b`, CronJobs décalés d'une demi-période (PR stoa #2842). À savoir : le code d'erreur passe de 503 à **500** — un client malchanceux reçoit une erreur applicative de quelques secondes au lieu d'une indisponibilité franche de plusieurs minutes. Détail : `SPIKE-wm-repliques-decalees-ignite.md`. |
