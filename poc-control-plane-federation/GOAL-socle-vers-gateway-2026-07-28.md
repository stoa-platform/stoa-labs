...
**État : FERMÉ le 2026-07-29.** Porte et contre-épreuve vertes — builds 7 (vert), 8 (rouge, sabotage) et 9 (vert, restauration) déclenchés sans action humaine, statuts `jenkins/probe` correspondants dans Gitea, puis **re-mesuré et rejoué par un quatrième push indépendant** (build 10, statut vert sur `f1e0f57`). Preuve détaillée : `docs/superpowers/plans/2026-07-28-f1-webhook-statut.md`, § « Preuve d'exécution ». Écart à traiter, **prioritaire sur F2-F5** : le matériel de descellement Vault a été perdu puis exposé en session ; le Vault en service tourne sur des clés neuves (mesuré), mais leur détention hors ligne reste à confirmer par l'exploitant — procédure non destructive au § « Incident Vault ». Un socle dont personne ne détient les parts retombe en panne définitive au premier `backup.yml`.
